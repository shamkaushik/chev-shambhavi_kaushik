...
(function (factory) {
	...
	function _render(self)
	{
		...
		for (var i=start, j=0; i <= end; j++, i += stepFunc(j)*60) {
			...
			if (settings.useSelect) {
                ...
                var newtimestring = timeString.substring(0,timeString.length-2)+" "+timeString.substring(timeString.length-2);
				row.text(newtimestring);
			}
	...
	{
		if (self.is('input')) {
            var newvalue = value.substring(0, value.length-2) + " " + value.substring(value.length-2);
			self.val(newvalue);
