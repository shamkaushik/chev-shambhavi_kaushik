...
! function($) {
    ...
    BootstrapTable.prototype.initHeader = function() {
        ...
        $.each(this.options.columns, function(i, columns) {
            ...
            $.each(columns, function(j, column) {
                ...
                html.push(text);
                html.push('<span class="fa"></span></div>');
        ...
        this.$container.parents(that.options.parentContainer).find(that.options.sortByDropdownId).on('changed.bs.select change', function(event) {
            var sort = $(this).val().split("-");
            that.options.sortName = sort[0];
            that.options.sortOrder = sort[1];
            // console.log(that.options.sortName + " - " + that.options.sortOrder);
            that.onSort(event);
        });
    ...
    //Initialiazing responsive view
    BootstrapTable.prototype.initResponsiveView = function() {
        if (this.options.responsive) {
            if ($(window).width() <= this.options.responsiveBreakPoint) {
                this.options.cardView = true;
                this.$container.addClass(this.options.responsiveClass);
            } else {
                this.options.cardView = false;
                this.$container.removeClass(this.options.responsiveClass);
            }
            this.initHeader();
            this.initBody();
    ...
    //End Initialiazing responsive view
    ...
    BootstrapTable.prototype.onSort = function(event) {
        ...
        if (event.type === "changed" || event.type === "change") {
            $this = this.$container.find('th[data-field="' + this.options.sortName + '"]');
        } else {
            ...
            var $sortDropdown = this.$container.parents(this.options.parentContainer).find(this.options.sortByDropdownId);
            if ($sortDropdown) {
                if ($sortDropdown.find('option[value=' + this.options.sortName + '-' + this.options.sortOrder + ']').length > 0) {
                    $sortDropdown.val(this.options.sortName + '-' + this.options.sortOrder).selectpicker('refresh');
                } else {
                    var sortOption = this.options.sortOrder === 'asc' ? 'desc' : 'asc';
                    $sortDropdown.val(this.options.sortName + '-' + sortOption).selectpicker('refresh');
                }
    ...
    // BOOTSTRAP EXTENSTION FOR REORDER ROWS
    // ==========================================
    var isSearch = false;
    ...
    BootstrapTable.prototype.onDrop = function(table, droppedRow) {
        ...
        if(newData.length > 1){
            tableBs.data('bootstrap.table').initBody(true); // After reorder refershing table body to change Data Index
        }
    };
}(jQuery);
